...
因为Java很多地方需要使用对象，比如集合类中，我们无法将int，double等数据类型放进去，因为集合的容器要求元素必须是对象；包装类还添加了属性和方法，丰富了基本类型的操作。

### String为什么不可变

1.String是被 `private final` 修饰的 `char` 数组，并且没有对外提供修改的方法。
2.不可变的好处：**线程安全**，同一个String可被多个线程共享；**支持hash映射和缓存**，String的hash值常被用到，如作为Map的key，不用重新计算hash值；**安全考虑**：URL，file path和密码之类的数据常用String保存；**字符串常量池优化**，String对象创建后，会被放入字符串常量池，下次需要同样的对象时可直接返回常量池中的引用。

### String，StringBuffer，StringBuilder

String不可变，Buffer和Builder可变；String和Buffer是线程安全的，Builder不是；Buffer内部使用 `synchronized` 同步。
...
### Spring Boot 启动类的注解，在启动过程中做了那些事，在ioc容器中的过程
